...
async def get_current_user_claims(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        return TokenData(id_user=id_user, username=username, role=role, id_hospital=id_hospital)
...
class PermissionGuard:
    ...
    def __call__(self, current_user: TokenData = Depends(get_current_user_claims), db: Session = Depends(get_db)):
