...
def get_sample_report(sample_id: int, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user_claims)):
    ...
    audit = models.AuditLog(user_id=current_user.id_user, action="GENERATE_REPORT", module="reports", entity=sample.sample_code, changes={"sample_id": sample.id, "sample_code": sample.sample_code, "patient_id": patient.id})
    db.add(audit)
    db.commit()
