...
# =====================================================================
if nav_selection == "samples Database":
    st.markdown('<div class="executive-card">', unsafe_allow_html=True)
...
if nav_selection == "Dashboard Matrix":
    ...
    st.markdown('</div>', unsafe_allow_html=True)
# ==========================================================================
# SAMPLES DATABASE (ÁREA 2)
# ==========================================================================
elif menu == "Samples Database":
    import sqlite3
    st.markdown('<div class="executive-card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">🧪 Sample Records & Permanent Database</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if not df_pacientes.empty:
            ...
            st.info("No active patient logs detected inside database.")
    except Exception:
        st.warning("Database layout initializing or empty...")
    st.markdown('</div>', unsafe_allow_html=True)

# ==========================================================================
# AI ANALYSIS HUB (ÁREA 3)
# ==========================================================================
elif menu == "AI Analysis Hub":
    st.markdown('<div class="executive-card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">🔬 AI Analysis Hub & Sequencer Validation</div>', unsafe_allow_html=True)
    
    col_qc1, col_qc2, col_qc3 = st.columns(3)
    with col_qc1: st.metric(label="🧬 Bisulfite Conversion", value="99.8%", delta="🟢 Optimal")
    with col_qc2: st.metric(label="📊 Mean Sequencing Depth", value="15,420x", delta="🟢 Certified")
    with col_qc3: st.metric(label="🧪 Sample Purity Score", value="1.84", delta="🟢 Pure Range")
    
    st.write("##")
    st.info("✅ RUN VALIDATION STATUS: VALID ASSAY. AI core prediction authorized.")
    st.markdown('</div>', unsafe_allow_html=True)

# ==========================================================================
# CLINICAL REPORTS (ÁREA 4)
# ==========================================================================
elif menu == "Clinical Reports":
    st.markdown('<div class="executive-card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">📈 Clinical Reports Audit Log</div>', unsafe_allow_html=True)
    st.caption("Obtain the uncompromised clinical validation dossier matching your pre-wetlab analytics.")
    
    st.write("##")
    st.download_button(
        label="📥 Download METHYLOX Corporate Dossier (PDF)",
        data=b"METHYLOX CONTENT SECURED",
...
# ==========================================================================
# SYSTEM SETTINGS (ÁREA 5)
# ==========================================================================
elif menu == "System Settings":
    st.markdown('<div class="executive-card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">⚙️ Engineering Core & Backend Diagnostics</div>', unsafe_allow_html=True)
    try:
        with open("motores.py", "r", encoding="utf-8") as file:
            codigo_backend = file.read()
        st.code(codigo_backend, language="python")
    except Exception:
        st.error("❌ No se pudo enlazar el visor en vivo con motores.py")
    st.markdown('</div>', unsafe_allow_html=True)
