...
# ----------------------------------------------------------------------------
    ...
    try:
        ...
        reports_data = res_reports.json() if res_reports.status_code == 200 else []
    except Exception:
        reports_data = []
    ...
    if not reports_data:
        ...
        pdf.cell(95, 5, f"Sample Asset ID: {str(datos_rep.get('muestra_id', ''))}", border=0)
        pdf.cell(95, 5, f"Verification Security Hash: {str(datos_rep.get('hash_seguridad', ''))}", border=0, ln=True)
        pdf.cell(95, 5, f"Authorized Operator Signature: {str(datos_rep.get('operador', ''))}", border=0)
        pdf.cell(95, 5, f"Server Transaction Timestamp: {str(datos_rep.get('fecha_analisis', ''))}", border=0, ln=True)
        pdf.ln(3)
        
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(190, 6, "2. ANONYMIZED PATIENT MOLECULAR DIRECTORY PROFILE", ln=True)
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(95, 5, f"Patient Context ID: {str(datos_rep.get('paciente_id', ''))}", border=0)
        pdf.cell(95, 5, f"Security Anonymous Code String: {str(datos_rep.get('nombre_codigo', ''))}", border=0, ln=True)
        pdf.cell(95, 5, f"Age: {str(datos_rep.get('age', ''))} Years", border=0)
        pdf.cell(95, 5, f"Biological Gender Parameter: {str(datos_rep.get('sexo', ''))}", border=0, ln=True)
        pdf.cell(190, 5, f"Medical Corporate Facility Node: {str(datos_rep.get('institucion', ''))}", ln=True)
        pdf.ln(3)
        
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(190, 6, "3. QUANTITATIVE EPIGENETIC METHYLATION READOUT (CORE SAMD ENGINE)", ln=True)
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(190, 5, f"Global Mean Methylation Beta Score (Multiplexed MOX Panel): {float(datos_rep.get('score', 0)):.4f}", ln=True)
        
        if float(datos_rep.get('score', 0)) >= 0.1000:
            pdf.set_text_color(220, 38, 38)
            pdf.set_font("Helvetica", "B", 9)
            pdf.cell(190, 5, f"ALGORITHMIC CLINICAL VERDICT: {str(datos_rep.get('clasificacion', ''))}", ln=True)
        ...
        if "TECHNICAL" in tipo_informe.upper():
            ...
            pdf.set_font("Helvetica", "", 9)
            pdf.cell(190, 5, f"Active Patent CRISPR Probes Panel Signatures: {str(datos_rep.get('guias_activas', ''))}", ln=True)
        ...
        try:
            ...
            if isinstance(final_pdf_payload, str):
                final_pdf_payload = final_pdf_payload.encode('latin1')
        except Exception:
...
# ----------------------------------------------------------------------------
    ...
    with st.form("universal_user_provisioning_form", clear_on_submit=True):
        ...
        target_hospital_id = st.number_input("Target Corporate Hospital ID Mapping Link", min_value=1, value=int(st.session_state.get('id_hospital', 1)))
