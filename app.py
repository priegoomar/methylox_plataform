...
# Variables ySliders en ejecución del Sidebar (Solo visibles en el Dashboard)
if nav_selection == "Dashboard Matrix":
    ...
    # VALORES ÓMICOS EN LÍNEA BASE PROTEGIDOS
    ct_dna_val = 0.2500
    slider_cpeb4 = st.sidebar.slider("CPEB4 (Base)", 0.0, 1.0, 0.45)
    slider_bcl2 = st.sidebar.slider("BCL2 (Base)", 0.0, 1.0, 0.62)
    slider_tp53 = st.sidebar.slider("TP53 (Base)", 0.0, 1.0, 0.18)
...
if nav_selection == "Dashboard Matrix":
    ...
    # BLOQUE MODULAR 1: MATRIZ DE PACIENTES
    ...
    # CONFIGURACIÓN AVANZADA DE SONDAS CRISPR
    st.write("---")
    ...
    if st.button(" Calcular Dictamen Clínico Multiplex", use_container_width=True):
        try:
            ...
            st.warning("Módulo 'motores.py' no encontrado. Mostrando simulación de Score: 0.0420")

    # MAPEO DE DENSIDAD DE COHORTE (PLOTLY)
    st.write("---")
    st.markdown("###  Cohort Density Mapping & Patient Positioning")
    st.caption("This interactive model projects the current patient's biomarker signal against the verified distribution curves of the TCGA-BRCA dataset.")
    ...
    # Cálculo dinámico basado en la sonda CRISPR ancla g1
    patient_y_pos = np.exp(-((g1 - 0.45) ** 2) / (2 * 0.15 ** 2)) if g1 > 0.2 else np.exp(-((g1 - 0.05) ** 2) / (2 * 0.03 ** 2))
    fig_cohort.add_trace(go.Scatter(
        x=[g1], y=[patient_y_pos], mode='markers+text', name='Current Patient Marker',
        marker=dict(color='#e74c3c', size=14, symbol='diamond', line=dict(color='white', width=2)),
        text=[" Current Patient"], textposition="top center", textfont=dict(family="Arial", size=12, color="#e74c3c")
    ))
    ...
    st.markdown("<p style='font-size: 11px; color: #7f8c8d; text-align: center;'> Digital Epigenetic Signature Mapping: Real-time tracking of sample hypermethylation cascades.</p>", unsafe_allow_html=True)

    # DESCARGA DE REPORTE INSTITUCIONAL
    st.write("---")
    ...
    st.download_button(
        label=" Download METHYLOX Corporate Dossier (PDF)", data=data_payload,
        file_name=pdf_nombre, mime="application/pdf", use_container_width=True, key="single_dossier_anchor_btn"
    )

    # PIPELINE DE CARGA MASIVA EXCEL / CSV
    st.write("##")
    archivo_cargado = st.file_uploader("Drag and drop your sequencer data matrix here", type=["csv", "xlsx"])
    if archivo_cargado is not None:
        try:
            import motores
            df_bulk = pd.read_csv(archivo_cargado) if archivo_cargado.name.endswith('.csv') else pd.read_excel(archivo_cargado)
            ...
            if all(col in df_bulk.columns for col in columnas_requeridas):
                ...
                if st.button(" Execute Bulk Processing & Secure to Database", use_container_width=True):
                    ...
                    st.toast(f" Storage secured: {registros_exitosos} records added.")
            else:
...
# --- PESTAÑA 2: SAMPLES DATABASE ---
    ...
    try:
        ...
        st.warning("Database layout empty or initializing... Mostrando repositorio centralizado vacío en tiempo real.")
...
# --- PESTAÑA 3: AI ANALYSIS HUB ---
    ...
    st.info("Procesador Alpha-Fold e indexadores cromatográficos activos de forma remota.")
    st.markdown('</div>', unsafe_allow_html=True)
...
# --- PESTAÑA 4: CLINICAL REPORTS ---
    ...
    st.markdown("###  Engineering Core & Backend Diagnostics")
    st.write("---")
    try:
        ...
        st.error(" No se pudo enlazar el visor con el core 'motores.py'")
    st.markdown('</div>', unsafe_allow_html=True)
...
# --- PESTAÑA 5: SYSTEM SETTINGS ---
    ...
    st.warning("Área restringida. Protocolos de encriptación AES-256 y llaves maestras cifradas.")
    st.info("Sección en desarrollo clínico secundario. Diagnóstico de logs e infraestructura en la nube.")
    st.markdown('</div>', unsafe_allow_html=True)
