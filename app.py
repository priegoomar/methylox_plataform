    ...
    if not st.session_state.jwt_access_token:
        ...
        if login_submit:
            if login_username and login_password:
                try:
                    ...
                    res = requests.post(f"{BACKEND_URL}/auth/login", data=payload_auth, timeout=5)
        ...
        st.markdown(
            ...
            <div style='background:#1E293B;padding:12px;border-radius:8px;'>
                ...
                <p style='color:#FFFFFF;font-weight:700;'>{st.session_state.operator_display_name}</p>
                <p style='color:#38BDF8;font-size:11px;'>ROLE: {st.session_state.user_role}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
...
# ----------------------------------------------------------------------------
    ...
    with p1:
        ...
        if st.session_state.user_role == "cls":
            ...
            try:
                res_h_dir = requests.get(f"{BACKEND_URL}/hospitals/directory", timeout=5)
...
# ----------------------------------------------------------------------------
# ⚙️ TAB 6: SYSTEM SETTINGS (KERNEL INTEGRITY AUDIT TRAIL MONITOR)
# ----------------------------------------------------------------------------
