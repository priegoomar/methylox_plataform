...
def api_get(path, params=None, timeout=10):
    try:
        ...
        return ApiResult(False, error=f"Backend connection error ({e.__class__.__name__})", status_code=None)
            ...
            if not username or not password:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.session_state.login_error = "Inactive user. Contact an administrator."
                    else:
                        st.session_state.login_error = f"Unexpected server error ({response.status_code})."
                except requests.exceptions.RequestException:
                    st.session_state.login_error = "Could not connect to the backend."
...
# ============================================================================
# PANTALLA SIN SESIÓN (RESTRICTED VIEW)
# ============================================================================

if nav_selection == "restricted":
    st.markdown('<div class="executive-card-white" style="text-align:center; padding:60px 40px; margin-top:40px;">', unsafe_allow_html=True)
    st.markdown("<h2 style='color:#0F172A; font-weight:800; font-size:24px; margin-bottom:10px;'>Restricted Access</h2>", unsafe_allow_html=True)
    st.caption("Please enter authorized clinician credentials in the sidebar to continue.")
    st.markdown('</div>', unsafe_allow_html=True)
...
# ============================================================================
    ...
    if not samples_result.ok:
        st.warning(f"Failed to load laboratory data: {samples_result.error}")
    ...
    st.caption("Note: Quality Control (QC) rate indicator was temporarily removed — it relied on a backend endpoint that does not exist yet (`/analysis/telemetry-summary`). It can be re-added once that endpoint is defined.")
    st.write("")
    ...
    with col_left:
        ...
        if samples:
            ...
            st.info("No samples registered yet." if samples_result.ok else "No data available (see warning above).")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col_right:
        ...
        if samples:
            ...
            st.markdown('<p style="color:#94A3B8; font-size:12px; text-align:center; padding:25px;">No sample data yet.</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
